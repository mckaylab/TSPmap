#' Compute genetic distances (in centiMorgans) by the Kosambi method
#'
#'    Compute genetic distances (in centiMorgans) by the Kosambi method, and assign a group number to the markers based on which cutpoint group they are in.
...
#' @usage computeKosambi(pathData, cutpoints)
...
#' @export
computeKosambi <- function(pathData, cutpoints=NULL)
{
  # Length of arrays.
  ...
  kosambicM <- array(0, dim=c(arrLen,2))

  # Label the groups.
  groupNum = 1

  # Start at the first cutpoint.
  kosambicM[cutpoints[1,2],1] = 0
  kosambicM[cutpoints[1,2],2] = groupNum
  ...
  for(i in (cutpoints[1,2]+1):arrLen)
  {
    ...
    {
      nextval = as.numeric(kosambicM[i-1]) + (0.25 * log( (1 + as.numeric(pathData[i-1,3])) / (1 - as.numeric(pathData[i-1,3])) ) * 100)
      kosambicM[i,1] = nextval
    }
    kosambicM[i,2] = groupNum
  }
  ...
  if(limit > 1)
  {
    for(i in 1:(cutpoints[1,2]-1))
    {
      ...
      {
        nextval = as.numeric(kosambicM[arrLen]) + 50 * log( 1 / (1 - as.numeric(pathData[arrLen,3])))
        kosambicM[i,1] = nextval
      }
      else
      {
  ...
  colnames(kosambicM) <- c("Kosambi (cM)", "group")

  newarray = cbind(pathData, kosambicM)
